...
\author{\href{https://github.com/Harsha081459/Question-Answering-System-with-a-Custom-Transformer-Encoder-and-Generative-Decoder}{\textbf{[GitHub]}} \quad $\mid$ \quad \href{https://iiitbac-my.sharepoint.com/:f:/r/personal/vishal_sriram_iiitb_ac_in/Documents/NLP_GROUP_18}{\textbf{[Weights -- OneDrive]}}}
\date{}
...
The pipeline shared all processing up to the encoder output, then branched into an extractive head or a generative decoder. This design ensured that the same passage understanding supported both span selection and text generation.
...
\end{tikzpicture}
\caption{System architecture with a concrete example. The shared encoder (Stage~2) processes the question-context pair into contextual representations $\mathbf{H} \in \mathbb{R}^{L \times 768}$. Two task-specific heads branch (Stage~3): extractive QA selects a span via n-best search, while generative QA decodes token-by-token.}
\end{figure*}

For the extractive path, two scalar heads scored each token as a possible start or end position, enabling rapid n-best span search. The generative path required a more complex structure: the decoder ingested previously generated tokens, attended to its own prefix via masked self-attention, queried the encoder memory via cross-attention, and predicted the next token autoregressively.
...
\end{tikzpicture}
\caption{Detailed encoder and decoder architecture. The encoder (top) is a
12-layer, 768-hidden BERT-style stack whose output branches into an
extractive QA head (start/end logits) and an encoder--decoder bridge
(Linear $768\!\to\!512$); the bridge feeds cross-attention inside the
4-block custom decoder (bottom), which produces the final answer tokens.}
\label{fig:encdec}
\end{figure*}
...
Evaluation used a two-tiered approach: a 1k/4k ``smoke evaluation'' subset from SQuAD v2 validation for rapid ablation testing, followed by the full 11,873-example validation set for final reported extractive metrics.

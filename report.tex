...
hard cap, whichever comes first. Decoding uses greedy search
(\texttt{beam=1}) with a length penalty of 0.4 to discourage
unnecessarily short outputs. Embedding weights are tied between the
decoder input embedding matrix and the final linear projection head,
reducing the parameter count and regularizing the output distribution.
